...
func DetectPlatforms() {
	...
	if 0 < len(errs) {
		// Only logging
		util.Log.Warnf("Failed to detect platforms. err: %v", errs)
	}
...
func Prepare() error {
	...
	if len(targets) == 0 {
		if 0 < len(nonTargets) {
			util.Log.Info("Servers on which dependent libraries were already installed")
	...
	util.Log.Info("The following servers need dependencies installed")
...
func Scan() []error {
	...
	if errs := checkRequiredPackagesInstalled(); errs != nil {
		util.Log.Errorf("Please execute with [prepare] subcommand to install required packages before scanning. %s", errs)
		return errs
	}
