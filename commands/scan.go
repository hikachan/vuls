...
func (p *ScanCmd) Execute(_ context.Context, f *flag.FlagSet, _ ...interface{}) subcommands.ExitStatus {
	...
	util.Log.Info("Detecting Server/Contianer OS... ")
	...
	// TODO remove
	//  if err := scan.CheckIfSudoNoPasswd(util.Log); err != nil {
	//      util.Log.Errorf("Failed to sudo with nopassword via SSH. Define NOPASSWD in /etc/sudoers on target servers")
	//      return subcommands.ExitFailure
	//  }
	...
	if errs := scan.Scan(); 0 < len(errs) {
		for _, e := range errs {
			util.Log.Errorf("Failed to scan. err: %s", e)
		}
		return subcommands.ExitFailure
	}
	fmt.Printf("\n\n\n")
